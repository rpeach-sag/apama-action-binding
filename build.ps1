...
param (
   ...
   [string]$output = "$(Split-Path $MyInvocation.MyCommand.Path -Parent)/output/ActionBinding",
   [switch]$forTest
)
...
& "$apamaBin/engine_deploy" --outputCDP "$output/cdp/ActionBinding.cdp" src
...
# Create the bundle
$files = & "$apamaBin/engine_deploy" --outputList stdout src | %{$_ -replace ".*[\\/]src[\\/]actions[\\/]","code/actions/"} | %{$_ -replace "\\","/"}
...
$cdpBundle = $bundleResult | %{$_ -replace "<%fileList%>","`t`t`t<include name=`"cdp/ActionBinding.cdp`"/>"} | %{$_ -replace "<%displayName%>", "ActionBinding CDP"}
$sourceBundle = $bundleResult | %{$_ -replace "<%fileList%>",$bundleFileList} | %{$_ -replace "<%displayName%>", "ActionBinding"}

New-Item -ItemType Directory -Force "$output/bundles" | out-null
# Write out utf8 (no BOM)
[IO.File]::WriteAllLines("$output/bundles/ActionBinding.bnd", $sourceBundle)
[IO.File]::WriteAllLines("$output/bundles/ActionBinding-CDP.bnd", $cdpBundle)
