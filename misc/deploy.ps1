...
$actionBindingHome = (Resolve-Path "$PSScriptRoot/..") -replace "\\","/"

$steFile = cat "$PSScriptRoot/template.ste"
$steFile = $steFile | %{$_ -replace "<%ACTION_BINDING_HOME%>",$actionBindingHome}
$steFile | Out-File -encoding utf8 "$sagInstallDir/Designer/extensions/actionbinding.ste"

[IO.File]::WriteAllLines("$actionBindingHome/actionbinding.properties", "ACTION_BINDING_HOME=$actionBindingHome")
